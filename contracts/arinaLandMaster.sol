...
contract landTycoonMaster is Manager, byt_str {
    using SafeMath for uint;
    using SafeMath16 for uint16;
    
    address public arina_contract;
    address public trade_address;
    address public landsMix_address;
...
////////////function///////////////////////////////////////////////////
    ...
    function rollDice() external{
        ...
        uint16 random = uint16(keccak256(abi.encodePacked(now, getRandom())));
    ...
    function playLotto() external{
        ...
        uint random = uint(keccak256(abi.encodePacked(now, getRandom())));
        uint random2 = uint(keccak256(abi.encodePacked(getRandom(), msg.sender)));

        address _address = inquire_slave_address(player_info[msg.sender].city);
        
        if(player_info[msg.sender].unmovable == false){
            (,uint8 _star) = slaveInterface(_address).inquire_domain_level_star(player_info[msg.sender].domain);
                if(_star <= 1){
                    _star = 1;
                }
            probability = probability.div(2**(uint(_star)-1));                   
            uint8[] memory buildings = slaveInterface(_address).inquire_tot_domain_building(player_info[msg.sender].domain);
    ...
    function ETH_count(uint8 _level, uint8 _star, uint8 action) public pure returns(uint){
        uint count;
        if(_level <= 1){
    	   count =  2 ether * 2**(uint(_star)-1) ;
    	}else if(_level > 1) {    
    	   count =  2 ether * 2**(uint(_star)-1)*(3**(uint(_level)-1))/(2**(uint(_level)-1)) ;
    	} 
        if (action == 1){
            ///payRoad
            return count;
    ...
    function payRent_ETH() external payable{
        ...
        (uint8 _level,uint8 _star) = slaveInterface(city_address).inquire_domain_level_star(domains_id);
        
        
        require(msg.value == ETH_count(_level, _star, 1));
    ...
    function buyLand_ETH() external payable{
        ...
        require(msg.value == ETH_count(_level, _star, 2)); 
        ...
        slaveInterface(city_address).transfer_master(msg.sender, domains_id); 
    ...
    function _payRent_ARINA(address _sender, uint _value) private{
        require(!all_stop);
        require(player_info[_sender].unmovable,"movable !!!");
        ...
        //require(domain_owner != 0x0, "Not space area!!!");
        (uint8 _level,uint8 _star) = slaveInterface(city_address).inquire_domain_level_star(domains_id);
        
        uint _payARINA_amount = payARINA_amount(_level, _star);
    	require(_value == _payARINA_amount,"value error!!!!");
        require(ERC20_interface(arina_contract).transferFrom(_sender, domain_owner, _value),"transferFrom ERROR!!!!!"); 
    ...
    function _buyLand_ARINA(address _sender, uint _value) private{ 
        ...
        require(domain_owner == 0x0, "Not space area!!!");
        require(_value ==  _buyLandARINA_amount,"payment error!!!");
        require(ERC20_interface(arina_contract).transferFrom(_sender, trade_address, _value),"arina.transferFrom ERROR!!!!!"); 
        
        slaveInterface(city_address).transfer_master(_sender, domains_id); 
        tradeInterface(trade_address).set_city_pool(_value,city);          
    ...
    function _build(address _sender, uint8 _building,uint _arina) private {
        require(!all_stop);
        require(player_info[_sender].build == true,"Cannot build!!!");
        uint16 city = player_info[_sender].city;
        uint16 domains_id = player_info[_sender].domain;
        
        address city_address = owner_slave[city];
		address domain_owner = ERC721_interface(city_address).ownerOf(domains_id);
		require(_sender == domain_owner,"Not Owner!!!");
		
		slaveInterface(city_address).domain_build(domains_id, _building);
    ...
    function reconstruction(uint8 _index, uint8 _building)public payable{
        ...
        address city_address = owner_slave[city];
		address domain_owner = ERC721_interface(city_address).ownerOf(domains_id);
		require(msg.sender == domain_owner, "not owner!!!");
        
        uint arina_price = inquire_type_price(_building);
        uint eth_price = arina_price.mul(10**8); 
        
        require(msg.value == eth_price,"eth_price error!!!");
        BuyLand_ETH_toTeam = msg.value.div(10).mul(7);
        manager.transfer(BuyLand_ETH_toTeam); 
        slaveInterface(city_address).reconstruction(domains_id, _index, _building);
        player_info[msg.sender].lotto = true;
        emit Reconstruction(msg.sender, _building,eth_price);
    }
    
    function domain_attribute(uint16 _city,uint16 _id, uint8 _index) public{
        require(msg.sender == landsMix_address);
        require(!all_stop);
        address city_address = owner_slave[_city];
        slaveInterface(city_address).set_domian_attribute(_id,_index);
    }
    ...
    function reward(uint8 _class, uint16 _city, uint16 _domains_id) public{
        ...
        if(inquire_owner(_city,_domains_id) != msg.sender){     
            require(!player_info[msg.sender].unmovable,"unmovable!!!"); 
        ...
        address city_address = owner_slave[_city];
        slaveInterface(city_address).domain_reward(_class, msg.sender, _domains_id);
    }  
    
    function all_reward(uint8 _class,uint16 _city) public{
        address city_address;
        city_address = owner_slave[_city];
        slaveInterface(city_address).domain_all_reward(_class, msg.sender);
    }
    
    function mayor_all_reward(uint16 _city) public{
        address city_address = owner_slave[_city];
        address _mayor = slaveInterface(city_address).inquire_mayor_address();
        require(msg.sender == _mayor, "only mayor!!!");
        slaveInterface(city_address).mayor_reward(msg.sender);
    }
    ...
    function exchange(uint16 _city,uint _value) payable public{
        uint rate;
        uint pool = tradeInterface(trade_address).inquire_pool(_city);
        
        uint exchange_master_ETH;
        uint exchange_player_ETH;
        uint exchange_Pool_ETH;
        require(msg.value == _value*10 ** 15);
        ...
        tradeInterface(trade_address).exchange_arina(exchangeArina,_city, msg.sender);
    ...
    function checkBuildingPlayer(uint16 _city,uint8 building) public view  returns(address[] ){  
        ...
        for(uint8 i=0 ; i<100; i++){
            uint8[] memory buildings = slaveInterface(owner_slave[_city]).inquire_tot_domain_building(i);
...
///////////////inquire//////////////////////////////////////////////////////
    ...
    function inquire_domain_level_star(uint16 _city, uint16 _domain) public view returns(uint8, uint8){
        address _address = inquire_slave_address(_city);
        return slaveInterface(_address).inquire_domain_level_star(_domain);
    }
    ...
    function inquire_city_totdomains(uint16 _index) public view returns(uint16){
        ...
        return  slaveInterface(_address).inquire_totdomains_amount();
    }
    ...
    function inquire_building(uint16 _slave, uint16 _domain, uint8 _index) public view returns(uint8){
        address _address = inquire_slave_address(_slave);
        return slaveInterface(_address).inquire_domain_building(_domain, _index);
    }
    
    function inquire_building_amount(uint16 _slave,uint8 _building) public view returns(uint8){
        address _address = inquire_slave_address(_slave);
        return slaveInterface(_address).inquire_building_limit(_building);
    }
    
    function inquire_tot_attribute(uint16 _slave, uint16 _domain) public view returns(uint8[5]){
        address _address = inquire_slave_address(_slave);
        return slaveInterface(_address).inquire_tot_domain_attribute(_domain);
    }
    
    function inquire_cooltime(uint16 _slave, uint16 _domain) public view returns(uint){
        address _address = inquire_slave_address(_slave);
        return slaveInterface(_address).inquire_domain_cooltime(_domain);
    }
    
    function inquire_tot_building(uint16 _slave, uint16 _domain) public view returns(uint8[]){
        address _address = inquire_slave_address(_slave);
        return slaveInterface(_address).inquire_tot_domain_building(_domain);
    }
    
    function inquire_own_domain(uint16 _city) public view returns(uint16[]){
        address _address = inquire_slave_address(_city);
        return slaveInterface(_address).inquire_own_domain(msg.sender);
    }
    ...
    function inquire_land_info(uint16 _city, uint16 _id) public view returns(uint8,uint8,uint8,uint8,uint8,uint8,uint8,uint8,uint8,uint8){
        address _address = inquire_slave_address(_city);
        return slaveInterface(_address).inquire_land_info(_city,_id);
    }
...
////arina 
    function receiveApproval(address _sender, uint256 _value, address _tokenContract, bytes _extraData) public{
        ...
        if (action == 0x0){ 
            ...
            require(_value == 100*10**8, "_value error!!!");
            ...
            if(checkPlayer.length != 0){
                ...
                require(ERC20_interface(arina_contract).transferFrom(_sender, trade_address, _value),"transfer error!!!");
                tradeInterface(trade_address).set_city_pool(_value,player_info[_sender].city);
            }
    		...
    		require(_value == build_value,"value error!!!"); 
            require(ERC20_interface(arina_contract).transferFrom(_sender, trade_address, _value),"transfer error!!!");
            tradeInterface(trade_address).set_city_pool(_value,player_info[_sender].city);
    ...
    function set_landsMix_address(address _landsMix_address) public onlymanager{ 
        landsMix_address = _landsMix_address;
    }
    ...
    function set_slave_mayor(uint16 _index, address newMayor_address) public onlymanager{
        ...
        slaveInterface(contract_address).transferMayorship(newMayor_address); 
    }
    
    function push_slave_address(address[] _address) external onlymanager{
        for(uint i = 0; i<_address.length; i++){
            owner_slave.push(_address[i]);
        }
        owner_slave_amount = owner_slave_amount.add(uint16(_address.length)); 
    } 
    ...
    function set_cooltime(uint16 _index, uint _cooltime) public onlymanager{
        address contract_address = owner_slave[_index];
        slaveInterface(contract_address).set_domain_cooltime(_cooltime); 
    }
